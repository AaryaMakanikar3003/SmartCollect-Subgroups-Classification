      ...
      {selectedCluster && (
        <div className="modal-overlay" onClick={() => setSelectedCluster(null)}>
          <div className="cluster-details-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <h3>Representative Conversations</h3> 

            <h3>Customer Behaviour</h3>
            <p>{selectedCluster.customer_behavior}</p>

            <h3>Payment Intent</h3>
            <p>{selectedCluster.payment_intent}</p>

            <h3>Common Patterns</h3>
            <ul>
              {selectedCluster.common_patterns?.map((p, i) => (
                <li key={i}>{p}</li>
              ))}
            </ul>
            ...
            <h3>Breakdown</h3>
            <ul>
              {selectedCluster.breakdown?.map((b, i) => (
              <li key={i}>
            ...
            <h3>Insights</h3>
            <ul>
              {selectedCluster.insights?.map((x, i) => (
                <li key={i}>{x}</li>
              ))}
            </ul>
            {(selectedCluster.sample_conversations || []).map((ex, index) => (
              <details key={index} className="conversation-card">
                <summary>Conversation {index + 1}</summary>
                <pre>{ex.preview}</pre>
              </details>
            ))}
            <button className="modal-btn modal-btn-primary close-btn" onClick={() => setSelectedCluster(null)}>
              Close
            </button>
          </div>
        </div>
