...
class _RateLimiter:
    """
    Enforces a minimum gap between consecutive LLM requests, across ALL
    threads. Complements (doesn't replace) the concurrency semaphore in
    direct_pipeline.py - the semaphore caps how many requests run at once,
    this caps how fast new ones can START, so we never fire faster than the
    gateway can actually keep up with, even if several requests are queued
    and finish quickly one after another.

    This only ever WAITS - it never speeds anything up. If a response takes
    longer than min_interval anyway (as most real requests do, based on
    today's ~0.7-1.5s average), this is a no-op and doesn't add any delay.
    """
...
def _call_llm(prompt: str, max_tokens: int, retries: int = 3, semaphore=None):
    ...
    for attempt in range(1, retries + 1):
        if semaphore:
            semaphore.acquire()
        try:
            _rate_limiter.wait()  # enforce the minimum gap right before sending
            print(f"    [naming] sending request (attempt {attempt}, max_tokens={max_tokens})...")
            start = time.time()
            result = client.chat.completions.create(
                model=_MODEL,
